...
const AppointmentForm = ({ appointment, patients, onSubmit, onCancel }: AppointmentFormProps) => {
  ...
  const handleChange = (
    ...
    if (name === 'patientId') {
      ...
      const dentist = mockUsers.find((d) => d.id === dentistId);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Cast to Appointment type (assumes all required fields are present after validation)
    onSubmit(formData as Appointment);
  };

  // Filter only dentists from users
  const dentists = mockUsers.filter(user => user.role === 'dentist' || user.role === 'admin');
